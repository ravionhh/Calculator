...
class MainActivity : AppCompatActivity() {
    ...
    fun plusMinusBtn(view: View){
        ...
        var cursorPos: Int = display.selectionStart
        var leftStr: String = oldString.substring(0, cursorPos)
        var rightString: String = oldString.substring(cursorPos)

        if (display.text.startsWith("-")){

            display.setText(String.format("%s%d%s",leftStr,"", rightString ))

        }
        else if(display.text.isNotEmpty() && ! display .text.equals(0)) {
            //oldString = display.text.getText()
            display.setText(String.format("%s%s", "-", leftStr))
            display.setSelection(cursorPos + 1)
        }
        else {
